...
if(!class_exists('___Response')){
    final class ___Response {
        ...
        private function from_array($response = []){
            ...
            if(!$this->code or !$this->message or $this->success != ___seems_successful($this->code)){
                $this->code = 500;
        ...
        public function __construct($response = null){
            ...
            $success = false;
            switch(true){
                case ___seems_response($response):
                    $code = $response['code'];
                    $data = $response['data'];
                ...
                case is_a($response, 'Requests_Response'):
                    ...
                    $success = ___seems_successful($code);
                    break;
                case is_wp_error($response):
                    $data = $response->get_error_data();
                    $message = $response->get_error_message();
                    break;
                case ___seems_wp_http_requests_response($response):
        ...
        public function to_wp_error(){
            if(is_wp_error($this->raw_response)){
                ...
                return ___error('http_request_failed', $this->message, [
                    'status' => $this->code,
                ]);
            }
        }
